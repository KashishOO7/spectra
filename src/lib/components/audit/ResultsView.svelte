<script lang="ts">
  ...
  import { maturityLabels, maturityColors, maturityDescriptions, EMOTIONAL_REGISTER_LABELS } from '$lib/audit/constants.js';
  import { catTextClass, catBarClass } from '$lib/audit/helpers.js';
  ...
  export let onCategory: (category: string) => void;
  export let onTakeQuiz: () => void;

  // Score sparkline — builds from timeline events that have score_after recorded.
      ...
      {#each [...result.category_scores].sort((a, b) => b.score - a.score) as cat}
        <div>
          <div class="flex items-center justify-between mb-1.5">
            <button type="button"
              on:click={() => onCategory(cat.category)}
            ...
            <div class="flex items-center gap-3">
              ...
              <span class="text-xs font-mono font-semibold w-10 text-right {catTextClass(cat)}">
                {cat.score}%
              </span>
            </div>
          </div>
